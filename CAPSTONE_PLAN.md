...
- **PAT 암호화**: Jasypt `StringEncryptor`로 암호화 후 DB 저장. 암호화 키는 DB에 저장하지 않고 **환경변수(`AES_SECRET_KEY`)로 주입** (로컬: `.env`, 배포: EC2 환경변수)
...
-- 댓글 (created_at, updated_at은 BaseEntity가 자동 관리)
comments: id, card_id, user_id, content, is_deleted
...
"PAT Jasypt 암호화 설정 및 GithubService 암호화/복호화 구현해줘 (키는 환경변수 AES_SECRET_KEY)"
